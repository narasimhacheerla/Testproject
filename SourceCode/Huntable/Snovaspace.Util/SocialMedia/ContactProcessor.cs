...
namespace Snovaspace.Util.SocialMedia
{
    ...
    public class ContactProcessor
    {
        ...
        public List<ContactDetails> serializeToObject(string contactsString)
        {
            ...
            {
                string[] individualArray = contactsArray[i].ToString().Split('~');
                ContactDetails CD = new ContactDetails();
                CD.name = individualArray[0].ToString();
                CD.email = individualArray[1].ToString();
                list_CD.Add(CD);
            }
            return list_CD;
        }
        ...
        public string doOAuthProcessing(string ServiceName, string token, string tokenSecret, string tokenVerifier, string consumerKey, string consumerSecret, string callbackUrl)
        {
            ...
            getParameters += "&CallbackUrl=" + encodeMe(callbackUrl) + "&Token=" + encodeMe(token) + "&TokenSecret=" + encodeMe(tokenSecret) + "&TokenVerifier=" + encodeMe(tokenVerifier);
        ...
        {
            string getParameters = "ServiceName=" + encodeMe(ServiceName) + "&ConsumerKey=" + encodeMe(consumerKey) + "&ConsumerSecret=" + encodeMe(consumerSecret);
            getParameters += "&CallbackUrl=" + encodeMe(callbackUrl) + "&PolicyUrl=" + encodeMe(policyUrl);
            string response_ = makeApiRequestGet(getParameters);
            return response_;
        }
        #endregion
        ...
        {
            string postParameters = "servicename=" + serviceName + "&signature=" + signatureKey + "&username=" + userName + "&password=" + password + "&format=" + format;
            if (serviceName.ToLower() == "outlook")
                postParameters = "servicename=" + serviceName + "&signature=" + signatureKey + "&filename=" + userName + "&CSVType=" + password + "&format=" + format;
        ...
        public string doProcessing(string serviceName, string token, string tokenSecret, string tokenVerifier, string UID, string consumerKey, string consumerSecret, string signatureKey, string format)
        {
            ...
            string postParameters = "ServiceName=" + serviceName + "&Token=" + token + "&TokenSecret=" + tokenSecret + "&TokenVerifier=" + tokenVerifier;

...
namespace eStreamChat
{
    public class MessengerCommand : IHttpHandler
    {
        ...
        public ChatRequest UpdateMessengerPresence(HttpContext context, User user)
        {
            ...
            var request = messengerProvider.GetChatRequest(user.Id);

            if (request != null)
            {
                request.ChatRequestMessage = String.Format("<b>{0}</b><br/> wants to chat with you!",
        ...
        {
            var request = messengerProvider.GetChatRequest(toUserId);
            ...
            if (rejected)
            {
                string message = String.Format("{0} rejected your chat request", request.ToUsername);
            ...
            {
                var message = String.Format("{0} accepted your chat request", request.ToUsername);

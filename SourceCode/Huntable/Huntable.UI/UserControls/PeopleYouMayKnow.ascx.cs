...
{
    public partial class PeopleYouMayKnow : System.Web.UI.UserControl
    {
        ...
        {
            LoggingManager.Debug("Entering LoginUserId - PeopleYouKnow");

            var loginUserId = Common.GetLoggedInUserId(Session);
            if (loginUserId != null) return loginUserId.Value;

            LoggingManager.Debug("Exiting LoginUserId - PeopleYouKnow");
        ...
        public void GetMutualFollowersList()
        {
            if (LoginUserId != 0)
            {
                using (var context = huntableEntities.GetEntitiesWithNoLock())
                {
                    ...
                    List<MasterPeople> mutualFollowersList;
                    if (mutualFollowersCount.Any())
                    {
                        mutualFollowersList =
                    ...
                    LoggingManager.Debug("Mutual users count in people youy may know is " + distinctMutualUsersList.Count());

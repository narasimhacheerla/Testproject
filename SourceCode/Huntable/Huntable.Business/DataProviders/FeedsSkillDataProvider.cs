...
{
    public class FeedsSkillDataProvider : DataProviderBase
    {
        public override IList<dynamic> GetItems(huntableEntities context, string searchContains, string startsWith, int pageIndex, int pageSize)
        {
            ...
            List<PreferredFeedUserSkill> selectedSkills = context.PreferredFeedUserSkills.ToList();
        ...
        public override void UpdateItems(List<int> newlyCheckedList, List<int> newlyUncheckedList)
        {
            using (var context = huntableEntities.GetEntitiesWithNoLock())
            {
                ...
                {
                    context.AddToPreferredFeedUserSkills(new PreferredFeedUserSkill { UserId = UserId, MasterSkillId = newlyCheckedItem });
                }

                List<PreferredFeedUserSkill> allUserSkills = context.PreferredFeedUserSkills.ToList();
